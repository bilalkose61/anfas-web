...
const stats = [
  ...
  { valueTr: "5.000+",   valueEn: "5,000+",   labelTr: "Katılımcı Firma",    labelEn: "Exhibitor Companies" },
  { valueTr: "81",       valueEn: "81",        labelTr: "İlden Katılım",      labelEn: "Provinces" },
  { valueTr: "30+",      valueEn: "30+",       labelTr: "Yıllık Deneyim",     labelEn: "Years of Experience" },
];

const pages = Array.from({ length: 13 }, (_, i) => `/reklam-sponsor/sayfa-${String(i + 1).padStart(2, "0")}.jpg`);

export default function SponsorshipPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="bg-white min-h-screen font-sans">
      ...
      <section className="relative h-[70vh] min-h-[500px] w-full flex items-end overflow-hidden bg-[#020617]">
        <img
          src="/images/sponsorship-cover.jpg"
          alt={tr ? "Sponsorluk" : "Sponsorship"}
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 pb-16 md:pb-24">
          ...
          <h1 className="text-5xl sm:text-7xl md:text-8xl font-black text-white tracking-tighter uppercase leading-[0.85]">
            {tr ? "Sponsorluk" : "Sponsorship"}
          </h1>
          <p className="text-white/40 text-sm md:text-base font-medium mt-6 max-w-md leading-relaxed">
            {tr
              ? "Markanızı Türkiye'nin önde gelen fuar merkezi ile büyütün"
              : "Grow your brand with Turkey's leading fair center"}
          </p>
        </div>
      </section>
            ...
            <div className="relative z-10 p-8 md:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div>
                ...
                <h3 className="text-white text-2xl font-black tracking-tight uppercase mb-2">
                  {tr ? "Sponsorluk Fırsatları" : "Sponsorship Opportunities"}
                </h3>
                <p className="text-slate-400 text-sm leading-relaxed">
                  {tr
                    ? "Sponsorluk paketleri ve fırsatlar için bizimle iletişime geçin."
                    : "Contact us for sponsorship packages and opportunities."}
                </p>
              </div>
              <a
                href={`/${locale}/contact`}
